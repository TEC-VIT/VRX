...
class GpsGoal():
  def __init__(self):
    global pub
    pub = rospy.Publisher('Station_Location',Twist,queue_size=10)
    rospy.init_node('gps_goal')
    rospy.Subscriber('/vrx/station_keeping/goal', GeoPoseStamped, self.gps_goal_pose_callback)
  ...
  def do_gps_goal(self, goal_lat, goal_long, z=0, yaw=0, roll=0, pitch=0):
    ...
    y, x = calc_goal(self.origin_lat, self.origin_long, goal_lat, goal_long)
  ...
  def gps_goal_pose_callback(self, data):
    ...
    long = data.pose.position.longitude
    z = data.pose.position.altitude
    quat = (
  ...
  def publish_goal(self, x=0, y=0, z=0, yaw=0, roll=0, pitch=0):
    rospy.loginfo("The goal position (x,y) is : {:.3f}, {:.3f} m.".format(x, y))
    rospy.loginfo("The goal orientation (r,p,y) is : {:.3f}, {:.3f}, {:.3f} deg".format(roll*(180.0/math.pi),pitch*(180.0/math.pi),yaw*(180.0/math.pi)))
    rate = rospy.Rate(1)
    angles=Twist()
    angles.linear.x=x
    angles.linear.y=y
    angles.linear.z=z
    angles.angular.x=roll
    angles.angular.y=pitch
    angles.angular.z=yaw
